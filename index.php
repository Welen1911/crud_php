	...
	<head>
		...
		<Script>
			...
			function apagar(id, tarefa) {
				location.href = `tarefa_controller.php?acao=apagar&id=${id}&tarefa=${tarefa}&pag=index`
			}
			function RealizarTarefa(id) {
				location.href = `tarefa_controller.php?acao=realizar&id=${id}&pag=index`
			}
		</Script>
	</head>
						...
						<div class="row">
							<div class="col">
								...
								for ($x = 0; $x<sizeof($listaTarefas); $x++) {
									if ($listaTarefas[$x]["status"] == "pendente") {
										$cont++;
									?>
									<div class="row mb-3 d-flex align-items-center tarefa">
										<div id="tarefa_<?= $listaTarefas[$x]['id']?>" class="col-sm-9"><?php echo $listaTarefas[$x]["tarefa"] ?></div>
										<div class="col-sm-3 mt-2 d-flex justify-content-between">
											...
											<i class="fas fa-check-square fa-lg text-success" onclick="RealizarTarefa(<?=$listaTarefas[$x]['id']?>)"></i>
										</div>
									</div>
									<?php }} if ($cont == 0) {?>
										<div class="col-sm-9">Você não tem tarefas pendentes!</div>
										<?php }?>
